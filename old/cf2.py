...
app.layout = html.Div([
    html.Div([
        ...
        dcc.Input(id='newCrypto', debounce=True, value='', type='text'),
        html.Br(),
        # html.Label('Период'),
        dcc.RadioItems(
            id='Period',
...
def update_graph(newCrypto, crypto, period, days, act, but, maxvols, intervals):
    global df, cry, refr, maxvols_g, act_g, df_exch
    refr = refresh[period]
    bars = days
    sbars = 1
    print('>'+newCrypto+'<')
    ...
    # print((df['Volume'][maxv]/df['Volume'][maxv].max()*20).astype('int64'))
    # print('===')
    # print(df['Volume'][maxv].rank(method='max'))
    ...
    fig.update_layout(
        title=f"Crypto-flash2 {exchange} {crypto} {bars}*{period}={days}D  SMA(30+60)  Vol Down:{Vols:,.0f} Vol Up:{Voll:,.0f}",
        xaxis_title="Время",
