...
"""
cache = diskcache.Cache("/tmp/ff/cache_bbol_03")
...
VERSION = f'BTC Bunch of Lines #03, Plotly V{VER_P}, Dash V{VER_D}, Bootstrap V{VER_B}'
...
def update_chart(data, end_price, n, sma_level,
    ...
    print('btc_df')
    print(btc_df)
    btc_df.loc[:, f'wvwma_f'] = wvwma(btc_df.close, btc_df.vol, length=sma_level)
    btc_df[f'sma_f'] = sma(btc_df.close, length=sma_level)
    # Price SMA
    btc_df['sma'] = sma(btc_df.close, length=sma_period_price)
    btc_df.set_index(keys='time', inplace=True)
    # df = df[-vis_limit:]
# TODO Использовать JOIN
    ...
    if price_sma:
        fig.add_trace(
            go.Scatter(
                x=df.index, y=btc_df['sma'], mode='lines', name=f'Price SMA',
                # hoverinfo='none',
                line=dict(
                    width=1,
                    color='white',
    ...
    fig.add_trace(
        go.Scatter(
            x=df.index, y=btc_df[f'wvwma_f'], mode='lines', name=f'WVWMA({sma_level})',
            # hoverinfo='none',
            line=dict(
                width=4,
                color='blue',
    ...
    fig.add_trace(
        go.Scatter(
            x=df.index, y=btc_df[f'sma_f'], mode='lines', name=f'SMA({sma_level})',
            # hoverinfo='none',
            line=dict(
                width=4,
                color='red',
...
if __name__ == '__main__':
    app.run_server(host='0.0.0.0', port=8053, debug=False, use_reloader=True)
